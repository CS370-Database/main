    ...
    if(mysqli_connect_errno()){
        ...
        try {
            ...
            foreach($lines as $line) {
                ...
                $sql_select = 'SELECT * FROM critic WHERE Fname="'.$fname.'" AND Lname="'.$lname.'"';
                $result = $con->query($sql_select);
                if($result->num_rows > 0) {
                    $sql_update = 'UPDATE critic SET Gender="'.$gender.'" WHERE Fname="'.$fname.'" AND Lname="'.$lname.'"';
                    $update_counter++;
                    $con->query($sql_update);
                }
                else {
                    $sql_insert = 'INSERT INTO critic (Fname, Lname, Gender) VALUES ("'.$fname.'","'.$lname.'","'.$gender.'")';
                    $insert_counter++;
...
<p1> Upload a CSV file with the new critic data to add the data to the Critic Table or update existing records.</p1>
<br>
<br>
<?php
if($import_attempted){
